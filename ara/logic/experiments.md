...
| E02 | 受控实验 | 四级 autonomy 在相同输入上的对比 | 未开始（**已降级为非阻塞**，见 C12） |
...
**Verifies**: C03（提供分层维度）、C10（L4 臂的前缀测量）；回答 open question
`Necessity-of-agentic-execution`

> **（2026-09-01）本实验已从「项目前提的阻塞项」降级为「设计余量的探究」。** 理由是 C12：
> `liu2026ara` 的 ARA Compiler 表明这类 workload 在第三方手里就是以 agent 形态部署的，
> 因此**研究对象的真实性不再取决于本实验的结果**。E02 现在问的是"这类系统能省下多少
> agency"，而不是"本项目是否站得住"。**这不改变本实验的设计，只改变它的优先级**——它
> 不再挡在 E01 与主线方法工作前面。
