...
> **历史参考（2026-09-09 保留）**：这份笔记从旧 CachePlan 方向评价论文；其中 verdict、OQ 关联和研究启示不代表新方向的判断。项目现已转向论文推理式抽取与文献资源库构建。
...
- **OQ3 的形态参考**：它验证“固定骨架 + 槽位填充式 agency”这一业务形态；但预定义计划与动态兜底本身不能证明 prompt/KV 动态复用，仍须与等价静态模板选择作强对照。
