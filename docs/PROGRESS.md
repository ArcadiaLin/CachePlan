...
| OQ3：什么样的 Paper-for-Agents workload 能检验动态跨 run 复用？ | **OPEN** | [open-questions/Multi-run-workloads.md](open-questions/Multi-run-workloads.md) | `2026-Helium`; `2026-AgenticScholar`; `2026-AlignedServe` | 2026-09-04 提出。P4A v1 是真实轨迹与动态边界案例，但其“一篇论文一条长 session”形态、早期私有输入和不充分的质量评测使其不能担任方法效果 benchmark。OQ3 要求先构造具真实业务依赖、独立质量 contract、版本化 fixture 与可观测 root/cohort/length/arrival 结构的 workload；并将 prompt/KV、artifact/result、plan 三层复用分开记账。任何方法贡献仍必须报告为 $A3-A2$，不能把静态布局或 artifact cache 的收益算入 CachePlan。 |
